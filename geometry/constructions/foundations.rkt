...
(define-construction perpendicular-bisector
  ...
  (step (caption "The circles meet at " P " and " Q ".")
    [(P Q) (intersections cA cB)])
  (step (caption "Draw the line through " P " and " Q ".") [m (line P Q)])
  (step "This line is the perpendicular bisector of the segment."
    (deemphasize cA cB) (hide base) (hide-label P Q))
...
(define-construction erect-perpendicular
  ...
  (layout (prefer (distance P Q)
                  (/ (distance (start-point l) (end-point l)) 3)))
  (step (caption "Choose " Q " on the line, different from " P ".")
    (show l) [Q (choose (point-on l #:except P))])
  (step (caption "Draw a circle centred at " P " through " Q ".") [c (circle P Q)])
  (step (caption "Let " R " be the other intersection with the line.")
    [R (intersection c l #:other-than Q)])
  (step (caption "Draw equal circles centred at " Q " and " R ".")
    [cQ (circle Q R)] [cR (circle R Q)])
  (step (caption "Call one of the circle intersections " S ".")
    [S (intersection cQ cR #:side-of l 'left)])
  ...
  (step "It passes through the given point at a right angle."
    ...
    (deemphasize c cQ cR) (hide-label Q R S))
...
(define-construction copy-angle
  ...
  (step (caption "Let " P " be its intersection with the target ray.")
    [P (intersection cO target)])
  (step (caption "Set the compass to " U D ", and draw a circle centred at " P ".")
    [cP (circle P #:radius (length chord))])
  (step "Use the intersection on the chosen side of the target ray."
    [Q (intersection cO cP #:side-of target side)])
  (step (caption "Draw the ray from " O " through " Q ".") [r (ray O Q)])
  (step "The new angle equals the source angle."
    [angles (marker (equal-angle source (angle P O Q)))]
    (deemphasize first-arm second-arm c cO cP chord) (hide-label U D P Q))
  (assert (equal-angle source (angle P O Q))
          (side-of? Q target side))
  (result r))
