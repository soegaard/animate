...
There are **63 test cases** across seven test files, containing multiple
assertions per case:

| File | Cases | Coverage |
|---|---:|---|
| `math-test.rkt` | 9 | Euclidean constructions, intersections, transforms, tangency, empty/overlapping loci, selectors, clipping. |
| `dsl-test.rkt` | 13 | Checked graph, type/arity/reference errors, free/derived distinction, typed helpers, hygiene, multi-results, expansion, cardinality and preconditions. |
...
| `theme-test.rkt` | 8 | Units, family/state composition, exact overrides, nested stroke rules, inheritance, paint channels, invalid properties. |
| `timeline-test.rkt` | 11 | Initial state, labels, no label flash, persistent state, transient highlights, seeking, no-ops, helper narration. |
| `drawing-test.rkt` | 5 | Dash runs, clipping, deterministic labels, explicit label placement, helper display names. |
| `animate-test.rkt` | 8 | Native tokens, exact colors, scene clock, arbitrary-time sampling, cosmetic widths, aspect checks, all three examples, PNG/subtitle/still-index output. |
...
# Compile the public modules and the three examples.
...
# Render the three step-by-step visual probes.
...
to the prior style. The bisector example should visibly expand the typed helper,
