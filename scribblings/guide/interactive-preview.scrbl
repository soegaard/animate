...
encoding a movie. It uses the same sampling and drawing as the renderer.
Run a GUI-capable Racket such as GRacket for this step.
@; introduces: preview

@section{Open the source program you just learned}

From the repository root, open the maintained example:
@verbatim{(require animate/preview)
(open-program-preview "examples/source-block-hot-reload.rkt" 'hot-reload-demo)}
...
@section{Review a short interval}

Use Play and the playhead to look at the motion. Shift-drag marks a review range;
Play range runs it once and Loop range repeats it. The block selector jumps to
the named blocks in the source program. Section and cue selectors appear only
when that kind of information exists.
...
@section{Inspect an object}

Select an object to inspect its identity and properties. Colored inspection
outlines and dependency arrows belong to the preview. They do not become objects
in the Scene or appear in the final movie.
