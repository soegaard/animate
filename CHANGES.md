...
- Added `stagger-map`, an eager list/vector-to-`lagged-start` composition
  helper. Factories run exactly once in source order and may receive the stable
  original source index; forward and reverse scheduling reuse the existing
  lag-ratio timing, endpoint, conflict, and random-access sampling semantics.
...
- Added `reveal-subsets`, an eager subset-progression composition. Its
  cumulative mode is ordinary staggered entry; its one-at-a-time mode starts a
  previous target's normal fade-out alongside each later scheduled entry.
...
- Added `typewrite`, a deterministic text-introduction request with
  grapheme/word/line/span fronts. Partial Pict frames are clipped from the
  final shaped layout, never generated by reflowing prefixes. Rich spans,
  explicit lines, and measured word wrapping use prepared final-token geometry;
  rotated or non-unit-scaled text reports its unsupported interior-mask
  capability explicitly rather than silently changing text placement. Optional
  cursor presentation is renderer-local, uses the prepared reveal frontier,
  and cleans up at the exact restored endpoint.
...
- Added `reveal-formula-parts`, an eager semantic formula-selection mapper.
  Named parts, explicit paths, and root-relative selections lower through
  `stagger-map` using an author-supplied concrete request factory.
