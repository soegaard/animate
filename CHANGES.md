...
- Corrected mapped composition semantics. `stagger-map` now accepts a deferred
  ...
  the explicit list/vector factory helper. Delay plans now make index, lead-in,
  and frozen-position timing independent of target ordering.

- Added exact affine/opacity lifecycle requests: `enter` installs an absent
  visual from a relative appearance and restores its authored value at the
  endpoint; `leave` transforms a current visual to a relative appearance and
  either removes it exactly at completion or retains that final state.
  Lifecycle inspection reports the requested components without exposing the
  private collapsed-scale proxy.

- Corrected subset progression semantics. `reveal-subsets` is cumulative only;
  `crossfade-subsets` supplies explicit entry/hold/exit/overlap timing for
  one-at-a-time display.

- Added deterministic hard-clip reveal fronts and lifecycle effects.
  `linear-reveal-front` and `radial-reveal-front` are pure frozen-layout
  geometry; `reveal-in` introduces through the clip and restores its exact
  authored Visual, while `reveal-out` reverses the clip and removes the target
  exactly at completion. Translation, rotation, scale, and opacity continue to
  compose normally; incompatible tree-replacement components are rejected.

- Added `pulse`, a deterministic exact-endpoint target pulse with a finite
  cycle count. It writes only requested affine/opacity components and exposes
  its target, cycles, write set, and source-restoration action to inspection.

- Added `ripple`, an eager lagged composition of live-tracking temporary
  outline rings. Rings read target layout without rewriting the target, use
  deterministic helper IDs, and are absent at exact start and completion.

- Added `apply-wave`, a target-local sinusoidal deformation for supported
  ordinary geometry. It samples from immutable clip-start data, has zero
  endpoint displacement, and restores the exact source Visual at completion.

- Added `confetti`, a seed-explicit temporary particle overlay. Its immutable
  plan uses closed-form ballistic samples, exposes seed/count/helper metadata
  to inspection, and removes every helper exactly at completion.

- Corrected `typewrite` terminology and capability reporting. Its units are
  grapheme/run/line/span (`word` is reserved for true Unicode word boundaries).
  Pict partial rendering is now explicitly restricted to conservative final-run
  fragment masks; rich, wrapped, transformed, complex, bidi, and
  ligature/kerning-sensitive interior states fail clearly rather than pretending
  a prefix layout is exact. Optional cursor presentation remains renderer-local.

- Added `erase-text`, the exact-endpoint reverse text lifecycle. It retains
  the source at local start, decreases its deterministic semantic reveal front
  during the clip, and removes the target rather than leaving empty text. Its
  optional cursor is likewise limited to partial open-clip samples.

- Added `underline-sweep`, an inspectable semantic path decoration with a
  deterministic helper identity and explicit retained/temporary lifecycle.
  Its first frozen-layout slice documents its unrotated/unscaled constraint
  rather than claiming live target following.

- Added the composable `strike-through` companion decoration. It uses an
  independent inspectable helper line through the frozen text-box midpoint.

- Added `highlight-sweep`, a background semantic rectangle inserted behind its
  text target in immutable scene drawing order. Its default temporary lifecycle
  and optional retained state are explicit.

- Corrected `reveal-formula-parts` to use semantic local-start formula target
  references rather than eager path/factory expansion.

- Added immutable animation ordering policies (`forward-order`,
  `reverse-order`, `permutation-order`, and seed-explicit `shuffled-order`) to
  mapped staggering and subset reveals. Added eager `repeat-animation` and
  explicit-pair `ping-pong` macros, both lowering to ordinary successions.

- Added seed-explicit `camera-shake`, an eager primary-camera pan succession
  whose local offsets telescope to an exact final camera center.
